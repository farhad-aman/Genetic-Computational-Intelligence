...
class Player(pygame.sprite.Sprite):
    ...
    """
        ...
        nn_input = self.input_vector(screen_width, screen_height, obstacles, player_x, player_y)
        nn_output = self.nn.forward(nn_input.reshape(-1, 1))
        if nn_output > 0.5:
            self.change_gravity('right')
        else:
            self.change_gravity('left')

    def input_vector(self, screen_width, screen_height, obstacles, player_x, player_y):

        (obstacle_x, obstacle_y, player_x, player_y) = (0, 0, player_x / screen_width, player_y / screen_height)

        if len(obstacles) > 1:
            obstacle_x = player_x - obstacles[0]['x'] / screen_width
            obstacle_y = player_y - obstacles[0]['y'] / screen_height

        return np.array([player_x, player_y, obstacle_x, obstacle_y])
